...
const Dashboard: React.FC<DashboardProps> = ({ profile, customers, onNavigate, onLogout }) => {
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden text-slate-900" dir="rtl">
      <aside className="w-60 bg-slate-900 text-white flex flex-col hidden lg:flex">
        <div className="p-5 text-lg font-bold border-b border-slate-800 tracking-tight">
          Followit <span className="text-sky-400">360</span>
        </div>
        <nav className="flex-1 p-4 space-y-1.5">
          <button className="w-full flex items-center gap-2.5 p-2.5 rounded-lg bg-sky-500 text-white font-bold shadow-lg shadow-sky-500/20 text-[13px]">
            <LayoutDashboard className="w-4.5 h-4.5" /> דשבורד
          </button>
          <button onClick={() => onNavigate('journal')} className="w-full flex items-center gap-2.5 p-2.5 rounded-lg hover:bg-white/5 transition-standard text-slate-400 text-[13px]">
            <UsersIcon className="w-4.5 h-4.5" /> יומן לקוחות
          </button>
          <button onClick={() => onNavigate('profile')} className="w-full flex items-center gap-2.5 p-2.5 rounded-lg hover:bg-white/5 transition-standard text-slate-400 text-[13px]">
            <Edit3 className="w-4.5 h-4.5" /> פרופיל
          </button>
        </nav>
        <div className="p-4 border-t border-slate-800">
          <button onClick={onLogout} className="w-full flex items-center gap-2.5 p-2 rounded-lg text-slate-400 hover:text-rose-400 hover:bg-rose-400/5 transition-standard text-[13px]">
            <LogOut className="w-4.5 h-4.5" /> התנתקות
          </button>
        </div>
      </aside>
