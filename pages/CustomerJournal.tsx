...
import { 
  ...
  Table as TableIcon, FileText, CheckCircle2, AlertCircle, Save, Download, ChevronDown, ChevronUp, Check
} from 'lucide-react';
               ...
               <table className="w-full text-right border-collapse">
                 ...
                 <tbody className="divide-y divide-slate-100">
                   {filteredCustomers.length === 0 ? (
                     ...
                     filteredCustomers.map(customer => customer.policies.map((policy) => {
      ...
      {viewingCustomer && (
        <ViewCustomerModal 
          ...
          onUpdate={(updated) => { 
            ...
            // setViewingCustomer(null); // Keep open if you want to see changes, but usually users expect close after Save
          }}
        />
      )}
    </div>
...
/* --- Global Sub-Policy Definitions and Options --- */
...
/* --- Add/Edit Policy Configuration Sub-Component --- */
const PolicyConfigurator: React.FC<{
  ...
  const handleAdd = () => {
    if (activeTab === 'elementary') {
      ...
      // Note: we update state and call parent
...
const AddCustomerModal: React.FC<{ onClose: () => void, onSubmit: (c: Customer) => void, availableCompanies: string[] }> = ({ onClose, onSubmit, availableCompanies }) => {
  ...
  const handleAddPolicyToForm = () => {
    ...
    if (activeTab === 'private' && currentInputs.details.subPolicies) {
        totalCost = Object.values(currentInputs.details.subPolicies as Record<string, number>).reduce((a, b) => a + b, 0);
    }
...
/* --- View/Edit CustomerModal Overhauled --- */
const ViewCustomerModal: React.FC<{ customer: Customer, onClose: () => void, availableCompanies: string[], onUpdate: (c: Customer) => void }> = ({ customer, onClose, availableCompanies, onUpdate }) => {
  ...
  // UI states for adding new policies during edit
  const [activeTab, setActiveTab] = useState<InsuranceType>('private');
  const [currentInputs, setCurrentInputs] = useState({ 
    company: availableCompanies[0] || '', 
    cost: 0, 
  ...
  const handleAddPolicyDuringEdit = () => {
    if (!currentInputs.company) return alert('נא לבחור חברת ביטוח');
    ...
    setEditedCustomer({ ...editedCustomer, policies: [...editedCustomer.policies, newPolicy] });
    setCurrentInputs({ ...currentInputs, cost: 0, isAgentAppointment: false, premiumSchedule: [], details: { subPolicies: {} } });
  };
          ...
          <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-white shrink-0">
             ...
             <div className="flex gap-3">
                <button 
                   ...
                   className={`px-8 py-2.5 rounded-xl font-bold flex items-center gap-2 transition-all ${isEditing ? 'bg-emerald-500 text-white shadow-lg' : 'bg-sky-50 text-sky-600 hover:bg-sky-100'}`}
                >
                   {isEditing ? <><Save className="w-4 h-4" /> צפה בלבד</> : <><Edit3 className="w-4 h-4" /> ערוך לקוח</>}
                </button>
                <button onClick={onClose} className="p-2 hover:bg-slate-100 rounded-full transition-standard text-slate-400"><X /></button>
             </div>
          </div>
             ...
             {isEditing && (
               <div className="mb-12 animate-fadeIn">
                 <h3 className="text-lg font-bold text-slate-900 mb-6 flex items-center gap-2">
                    <Plus className="w-5 h-5 text-sky-500" /> הוספת פוליסה חדשה ללקוח זה
                 </h3>
                 <PolicyConfigurator 
                   activeTab={activeTab}
                   setActiveTab={setActiveTab}
                   ...
                   currentInputs={currentInputs}
                   setCurrentInputs={setCurrentInputs}
                   onAddPolicy={handleAddPolicyDuringEdit}
                   buttonLabel="הוסף פוליסה לרשימה"
                 />
               </div>
             )}
             ...
             <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                   {editedCustomer.policies.map(p => (
                     <div key={p.id} className={`p-8 rounded-[2.5rem] border-2 transition-all ${p.status === 'active' ? 'bg-white border-slate-100 shadow-sm' : 'bg-slate-50 border-slate-200 grayscale opacity-60'}`}>
                       <div className="flex justify-between items-start mb-6">
                          ...
                          <div className="flex flex-col items-end gap-2">
                            ...
                            {isEditing && (
                              <button onClick={() => handleRemovePolicy(p.id)} className="text-rose-400 hover:text-rose-600 transition-colors"><Trash2 className="w-4 h-4" /></button>
                            )}
                          </div>
                       </div>
                       
                       {/* Expanded Policy Detail / Editing */}
                       <div className="space-y-4">
                                 ...
                                 <div className="relative">
                                    <input 
                                      ...
                                      onChange={e => setEditedCustomer({
                                        ...editedCustomer,
                                        policies: editedCustomer.policies.map(pol => pol.id === p.id ? { ...pol, monthlyCost: parseFloat(e.target.value) || 0 } : pol)
                                      })}
                                      className="w-32 p-1.5 pr-8 border border-slate-200 rounded-lg text-xl font-bold bg-white" 
                                    />
                          ...
                          {/* Private Insurance Detailed Edit */}
                          {p.type === 'private' && p.details.subPolicies && (
                                         ...
                                         <div className="relative">
                                           <input 
                                             ...
                                             onChange={e => {
                                                ...
                                                setEditedCustomer({
                                                  ...editedCustomer,
                                                  policies: editedCustomer.policies.map(pol => {
                                                     if (pol.id === p.id) {
                                                        const subPol = { ...(pol.details.subPolicies || {}), [sub]: newVal };
                                                        const newTotal = Object.values(subPol).reduce((a, b) => (a as number) + (b as number), 0) as number;
                                                        return { ...pol, monthlyCost: newTotal, details: { ...pol.details, subPolicies: subPol } };
                                                     }
                                                     return pol;
                                                  })
                                                });
                                             }}
                                             className="w-20 p-1 pl-6 border border-slate-200 rounded bg-white text-left font-bold" 
                                           />
                                           <span className="absolute left-1 top-1 text-[10px] text-slate-300 font-bold">₪</span>
                                         </div>
                                       ) : (
                          ...
                          {/* Elementary detailed edit */}
                          {p.type === 'elementary' && (
                             <div className="bg-slate-50/50 p-4 rounded-xl border border-slate-100 space-y-3">
                                {isEditing ? (
                                   <div className="grid grid-cols-2 gap-3">
                                      <div>
                                         <label className="text-[9px] font-bold text-slate-400 block mb-1">קטגוריה</label>
                                         <select 
                                            value={p.details.elementaryCategory} 
                                            onChange={e => setEditedCustomer({
                                              ...editedCustomer,
                                              policies: editedCustomer.policies.map(pol => pol.id === p.id ? { ...pol, details: { ...pol.details, elementaryCategory: e.target.value, elementaryCoverage: ELEMENTARY_OPTIONS[e.target.value as keyof typeof ELEMENTARY_OPTIONS][0] } } : pol)
                                            })}
                                      ...
                                      <div>
                                         <label className="text-[9px] font-bold text-slate-400 block mb-1">כיסוי</label>
                                         <select 
                                            value={p.details.elementaryCoverage} 
                                            onChange={e => setEditedCustomer({
                                              ...editedCustomer,
                                              policies: editedCustomer.policies.map(pol => pol.id === p.id ? { ...pol, details: { ...pol.details, elementaryCoverage: e.target.value } } : pol)
                                            })}
                                            className="w-full p-2 border border-slate-200 rounded bg-white text-[11px]"
                                         >
                                            {ELEMENTARY_OPTIONS[p.details.elementaryCategory as keyof typeof ELEMENTARY_OPTIONS]?.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                                         </select>
                                      </div>
                                   </div>
                                ) : (
                                   <div className="text-xs text-slate-500 italic">
                                      {p.details.elementaryCategory} • {p.details.elementaryCoverage}
                                   </div>
                                )}
                             </div>
                          )}
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h5 className="text-xs font-bold text-slate-700 flex items-center gap-2"><TableIcon className="w-4 h-4 text-sky-500" /> טבלת התפתחות פרמיה (XLSX)</h5>
        <button onClick={() => fileInputRef.current?.click()} className="text-[10px] bg-white border border-slate-200 text-slate-600 px-3 py-1.5 rounded-lg font-bold hover:border-sky-500 hover:text-sky-600 transition-all">העלה קובץ אקסל</button>
      ...
      {data && (
        <div className="grid grid-cols-2 gap-4 animate-fadeIn bg-white p-4 rounded-xl border border-slate-200">
          <div>
            ...
            <select value={mapping.age} onChange={(e) => setMapping({...mapping, age: e.target.value})} className="w-full p-2 text-[10px] border border-slate-200 rounded-lg outline-none bg-white">
          ...
          <div>
            ...
            <select value={mapping.premium} onChange={(e) => setMapping({...mapping, premium: e.target.value})} className="w-full p-2 text-[10px] border border-slate-200 rounded-lg outline-none bg-white">
              <option value="">בחר...</option>
              {Object.keys(data[0]).map(c => <option key={c} value={c}>{c}</option>)}
            </select>
          </div>
          <button onClick={() => {
            ...
            setData(null);
          }} className="col-span-2 bg-sky-500 text-white py-2 rounded-lg font-bold text-[10px] shadow-md">טען נתונים</button>
        </div>
      )}
    </div>
  );
